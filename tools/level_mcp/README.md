...
Set `ASW_PROJECT_ROOT` only when the server is launched outside this checkout. Tools edit `levels/_drafts/` only; `publish_draft` explicitly copies a draft to tracked `levels/`. Read a draft first, then pass its `revision` as `expected_revision` to every mutation. Stale edits are rejected; revisions, rollback snapshots, and up to 20 local playtest reports survive server restarts.
