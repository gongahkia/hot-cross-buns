...
export function HealthPanel({ subject, connected, connectionProfile, status, syncProgress, openDiagnostics }: HealthPanelProps): React.JSX.Element {
  ...
  useEffect(() => {
    let current = true;
    if (connectionProfile.mode !== "managed") {
      setManagedService("available");
      return () => { current = false; };
    }
    setManagedService("checking");
    void managedServiceAvailable().then((available) => {
  ...
  return <section className="workspace-panel health-panel" aria-labelledby="health-heading">
    ...
    <ul className="health-list">
      {connectionProfile.mode === "managed" && <li><div><strong>Self-hosted service</strong><span>{managedService === "checking" ? "Checking server and database" : managedService === "available" ? "Server and database are reachable" : "The configured self-hosted service is unavailable"}</span></div>{state(managedService === "available")}</li>}
      <li><div><strong>Google authorization</strong><span>{connected ? connectionProfile.mode === "managed" ? "Active through your self-hosted server session" : "Active for this browser session" : connectionProfile.mode === "managed" ? "Sign in through your self-hosted service to sync" : "Select Sync to renew Google access"}</span></div>{state(connected)}</li>
