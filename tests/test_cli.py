...
def test_theme_commands_apply_bundled_and_complete_custom_themes(
    ...
    assert listed[-1]["rank"] == 60
    assert listed[-1]["name"] == "Poimandres"
    ...
    latest = json_data(invoke(runner, ["--json", "themes", "60"]), "themes.apply")
    assert latest["source"]["rank"] == 60
    assert latest["theme"]["preset"] == "Poimandres"
