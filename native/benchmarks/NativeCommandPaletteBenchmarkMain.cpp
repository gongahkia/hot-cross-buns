...
int main(int argc, char* argv[]) {
  ...
  QCoreApplication::setApplicationName(QStringLiteral("hcb_command_palette_benchmark"));
  ...
  const QCommandLineOption iterationsOption(
      ...
      QStringLiteral("Number of palette-open samples (1-20)."),
      QStringLiteral("count"),
      QString::number(kDefaultIterations));
  const QCommandLineOption timeoutOption(
      {QStringLiteral("timeout-ms")},
      QStringLiteral("Per-sample timeout in milliseconds (1-60000)."),
  ...
  if (!parser.isSet(executableOption) || !iterationsValid || iterations <= 0 ||
    ...
    QTextStream(stderr) << "Invalid command-palette benchmark arguments.\n";
    return 2;
  }

  QString error;
  const auto result =
      hcb::NativeCommandPaletteBenchmark::measure(parser.value(executableOption),
  ...
  QTextStream(stdout) << hcb::NativeCommandPaletteBenchmark::toJson(*result) << '\n';
  return 0;
}
