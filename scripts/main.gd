...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo and event.physical_keycode == KEY_F3 and rebinding_action.is_empty():
		...
		get_viewport().set_input_as_handled()
		return
	if event is InputEventKey and event.pressed and not event.echo and event.physical_keycode == KEY_F4 and rebinding_action.is_empty() and world_streamer:
		megastructure_debug_visible = not megastructure_debug_visible
		world_streamer.set_megastructure_debug_visible(megastructure_debug_visible)
...
func _load_expedition(level: Dictionary) -> void:
	...
	var interior: Dictionary = megastructure_descriptor.get("interior", {})
	var canonical_spawn := Vector3(float(approach[0]), float(interior.get("floor_y", 0)) + 1.55, float(approach[2]))
...
func _refresh_debug_hud() -> void:
	...
	if world_streamer:
		...
		streaming_text = "\nSTREAM %.1fms  MAX %.1fms  HITCH %d  LAST %.1fms\nBUILD A %.1f F %.1f D %.1f C %.1f  QUEUE A %d C %d F %d\nMEGA M %.1f S %.1f X %.1f T %.1f\nLAST A %.1f F %.1f D %.1f C %.1f\nLAST M %.1f S %.1f X %.1f T %.1f\nMEM %.1fMB  PAYLOAD %.1fMB" % [float(refresh.get("refresh_ms", 0.0)), float(telemetry.get("max_refresh_ms", 0.0)), int(telemetry.get("hitches", 0)), float(last.get("refresh_ms", 0.0)), float(refresh.get("active_chunk_build_ms", 0.0)), float(refresh.get("far_chunk_build_ms", 0.0)), float(refresh.get("feature_build_ms", 0.0)), float(refresh.get("collision_lod_ms", 0.0)), int(refresh.get("queued_active_chunks", 0)), int(refresh.get("pending_collision_lods", 0)), int(refresh.get("pending_features", 0)), float(refresh.get("macro_silhouette_ms", 0.0)), float(refresh.get("sector_shell_ms", 0.0)), float(refresh.get("structure_collision_ms", 0.0)), float(refresh.get("traversal_detail_ms", 0.0)), float(last_phases.get("active_chunk_build_ms", 0.0)), float(last_phases.get("far_chunk_build_ms", 0.0)), float(last_phases.get("feature_build_ms", 0.0)), float(last_phases.get("collision_lod_ms", 0.0)), float(last_phases.get("macro_silhouette_ms", 0.0)), float(last_phases.get("sector_shell_ms", 0.0)), float(last_phases.get("structure_collision_ms", 0.0)), float(last_phases.get("traversal_detail_ms", 0.0)), float(int(memory.get("static_memory_bytes", 0))) / 1048576.0, float(int(memory.get("minimum_payload_bytes", 0))) / 1048576.0]
	var balance_text := ""
	if bool(current_level.get("procedural", false)):
