...
static func step(archetype: Dictionary, animal_position: Vector3, player_position: Vector3, warning_issued: bool = false, territory_center: Vector3 = animal_position) -> Dictionary:
	var offset := animal_position - player_position
	var distance := offset.length()
	var territory_intrusion := bool(archetype.get("territorial", false)) and territory_center.distance_to(player_position) <= float(archetype.get("territory_radius", 0.0))
