...
func _initialize() -> void:
	...
	assert(not main.megastructure_descriptor.is_empty() and not (main.world_streamer.chunks as Dictionary).is_empty(), "expedition streamed megastructure missing")
	...
	main.debug_visible = true
	main._refresh_debug_hud()
	assert("STREAM" in main.debug_label.text and "MEGA" in main.debug_label.text, "streaming phase diagnostics missing")
	...
	var player_chunk: Vector2i = main.world_streamer.origin.chunk_at_local(main.player.global_position)
	var opening_chunk := main.world_streamer.chunks.get("%d:%d" % [player_chunk.x, player_chunk.y]) as Node3D
	var opening_route := opening_chunk.get_node_or_null("MegastructureTraversal/TraversalGuide") as MeshInstance3D if opening_chunk else null
	var megastructure_debug := opening_chunk.get_node_or_null("MegastructureTraversal/MegastructureDebug") as Node3D if opening_chunk else null
	...
	assert(opening_route != null and is_equal_approx(main.world_streamer.ground_height(main.player.global_position), 24.0), "expedition player settled outside the opening route")
	var f4 := InputEventKey.new()
	f4.physical_keycode = KEY_F4
	f4.pressed = true
	main._input(f4)
	assert(main.megastructure_debug_visible and megastructure_debug != null and megastructure_debug.visible, "F4 did not show megastructure debug geometry")
	main._input(f4)
	assert(not main.megastructure_debug_visible and megastructure_debug != null and not megastructure_debug.visible, "F4 did not hide megastructure debug geometry")
