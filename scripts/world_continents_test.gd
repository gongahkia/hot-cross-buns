...
func _initialize() -> void:
	...
	_assert_sample(world.sample(-4480.0, -6400.0, "region"), "region", 4, "oceanic", -0.22453233392212, -0.47618967780199, 5536.47484203411, 1.0)
	_assert_sample(world.sample(-7680.0, -7680.0), "local", 1, "continental", 0.22, 0.17108735396839, 0.0, 0.0)
...
func _assert_sample(sample: Dictionary, scale: String, factor: int, crust: String, bias: float, elevation: float, ocean_depth: float, shelf_proximity: float) -> void:
	...
	var expected := float(sample.get("continental_bias", 0.0)) + (float(sample.get("continent", 0.0)) - 0.5) * 0.72 + (float(sample.get("rough", 0.0)) - 0.5) * 0.24 * (1.0 - float(sample.get("stable_damping", 0.0))) * float(sample.get("relief_scale", 1.0))
