...
func _initialize() -> void:
	...
	var forecast := WEATHER.forecast(world, [{"route":"forward","distance":60.0,"cell":cell}, {"route":"ridge","distance":120.0,"cell":cell}], {"clock":30.0,"travel_speed":6.0})
	_expect(forecast == WEATHER.forecast(world, [{"route":"forward","distance":60.0,"cell":cell}, {"route":"ridge","distance":120.0,"cell":cell}], {"clock":30.0,"travel_speed":6.0}) and (forecast as Array).size() == 2 and float((forecast as Array)[1].eta_seconds) > float((forecast as Array)[0].eta_seconds) and float((forecast as Array)[0].risk) >= 0.0, "weather forecast drifted")
