...
func _initialize() -> void:
	...
	var warning: Dictionary = BEHAVIOR.step(boar, Vector3(20.0, 0.0, 0.0), Vector3(5.0, 0.0, 0.0), false, Vector3.ZERO)
	_expect(warning.state == "warn" and bool(warning.warning_issued) and BEHAVIOR.step(boar, Vector3(20.0, 0.0, 0.0), Vector3(5.0, 0.0, 0.0), true, Vector3.ZERO).state == "flee" and BEHAVIOR.step(boar, Vector3(20.0, 0.0, 0.0), Vector3(13.0, 0.0, 0.0), false, Vector3.ZERO).state == "flee", "territorial wildlife behavior drifted")
