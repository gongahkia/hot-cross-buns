...
func _initialize() -> void:
	...
	_expect(plates.cache_metrics() == {"hits": 1, "misses": 3, "evictions": 1, "size": 2, "capacity": 2}, "plate cache LRU metrics drifted")
	_assert_center(plates.center(0, -1), 1026967433, 418.16730633378, -93.168891066313, 0, -1)
	_expect(plates.cache_metrics() == {"hits": 1, "misses": 4, "evictions": 2, "size": 2, "capacity": 2}, "plate cache eviction order drifted")
	plates.clear_cache()
	_expect(int(plates.cache_metrics().get("size", -1)) == 0, "plate cache clear failed")
	var uncached = PLATES.new(20260730, 640.0, 0)
	uncached.center(0, -1)
	uncached.center(0, -1)
	_expect(uncached.cache_metrics() == {"hits": 0, "misses": 2, "evictions": 0, "size": 0, "capacity": 0}, "zero-capacity plate cache drifted")
