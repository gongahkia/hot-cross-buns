...
func request(chunk_x:int,chunk_z:int,lod:int=0)->int:
	token+=1;queue.append({"seed":seed,"chunk_x":chunk_x,"chunk_z":chunk_z,"lod":lod,"token":token});return token
func poll()->Array:
	if worker and not worker.is_alive(): completed.append(worker.wait_to_finish());worker=null;active={}
...
func wait_for_all()->Array:
	...
	while not queue.is_empty() or worker:
		if worker:
			result.append(worker.wait_to_finish());worker=null;active={}
