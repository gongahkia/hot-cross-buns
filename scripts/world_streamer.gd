...
func refresh(force: bool) -> void:
	...
	var center := Vector2i(floori(player.global_position.x / WorldGenerator.CHUNK_SIZE), floori(player.global_position.z / WorldGenerator.CHUNK_SIZE))
...
func _update_region() -> void:
	var region := generator.region_at(player.global_position)
...
func _build_chunk(chunk_x: int, chunk_z: int) -> Node3D:
	...
	root.position = Vector3(float(chunk_x) * WorldGenerator.CHUNK_SIZE, 0.0, float(chunk_z) * WorldGenerator.CHUNK_SIZE)
...
func _terrain_mesh(chunk_x: int, chunk_z: int, biome: String, family: String) -> ArrayMesh:
	...
	var step := WorldGenerator.CHUNK_SIZE / float(GRID)
	for z in range(GRID):
		for x in range(GRID):
			...
			for vertex in [a, c, b, a, d, c]:
				surface.set_uv(Vector2(vertex.x / WorldGenerator.CHUNK_SIZE, vertex.z / WorldGenerator.CHUNK_SIZE))
...
func _vertex(chunk_x: int, chunk_z: int, x: int, z: int, step: float) -> Vector3:
	...
	var sample := generator.sample(float(chunk_x) * WorldGenerator.CHUNK_SIZE + local_x, float(chunk_z) * WorldGenerator.CHUNK_SIZE + local_z)
...
func _add_features(root: Node3D, chunk_x: int, chunk_z: int, descriptor: Dictionary) -> void:
	...
	if resource_roll > 0.32:
		var kind := "scrap" if family in WorldGenerator.URBAN_FAMILIES else "wood"
...
func _add_resource(root: Node3D, chunk_x: int, chunk_z: int, kind: String, salt: int) -> void:
	...
	var ground := ground_height(root.global_position + Vector3(x, 0.0, z))
	var pickup: ResourcePickup = RESOURCE_PICKUP.new()
...
func _terrain_material(biome: String, family: String) -> StandardMaterial3D:
	var color := {"ocean": Color("#2e5d6b"), "coast": Color("#8f9b70"), "desert": Color("#aa9564"), "alpine": Color("#82919b"), "nival_zone": Color("#d5e5df"), "wetland": Color("#4b7665"), "temperate_forest": Color("#55794e"), "rainforest": Color("#3e7146")}.get(biome, Color("#5e7650"))
