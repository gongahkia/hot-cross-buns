...
func _initialize() -> void:
	...
	for lap in range(LAPS):
		for chunk_x in _route():
			...
			_expect(streamer.chunks.size() == 25 and streamer.far_chunks.size() == 96 and streamer.pending_chunks.size() <= 24, "Windows streaming window drifted on lap %d: %d/%d/%d" % [lap,streamer.chunks.size(),streamer.far_chunks.size(),streamer.pending_chunks.size()])
			_expect(streamer.chunk_cache.size() <= streamer.chunk_cache.capacity and int(memory.minimum_payload_bytes) > 0, "Windows streaming cache drifted on lap %d" % lap)
		main.photo_mode.toggle()
		_expect(main.photo_mode.active and not main.hud.visible, "Windows photo-mode entry drifted on lap %d" % lap)
		main.photo_mode.toggle()
		_expect(not main.photo_mode.active and main.hud.visible, "Windows photo-mode exit drifted on lap %d" % lap)
		var current_memory := int(main.world_streamer.chunk_memory_snapshot().static_memory_bytes)
		if settled_memory < 0:
			settled_memory = current_memory
		else:
			_expect(current_memory <= int(float(settled_memory) * 1.1) + MEMORY_GROWTH_ALLOWANCE, "Windows retained memory grew after return lap %d" % lap)
	_expect(rebases >= LAPS * 2, "Windows long-run route did not repeatedly rebase")
	main.queue_free()
	await process_frame
