...
This mode first recovers only explicitly marked residue in the dedicated resources, then runs six substantive checks: read/render; direct Task and Calendar create/update/delete; exact Google recurrence-line round-trip; supported metadata; a timed three-run create/update/sync/cleanup benchmark; and Command Palette Quick Add for both an event and a task. The recurrence check verifies Google's canonical `RRULE`/`EXRULE`/`RDATE`/`EXDATE` set survives an unrelated event rename. The metadata check verifies Calendar description, location, guest, reminder, timezone, color, visibility, transparency, and a basic recurrence rule, plus HCB task-planning metadata stored in Google Tasks notes. Quick Add intentionally opens the existing full editor, which then saves and syncs the item.
