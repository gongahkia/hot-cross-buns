...
This mode first recovers only explicitly marked residue in the dedicated resources, then runs seven substantive checks: read/render; direct Task and Calendar create/update/delete; exact Google recurrence-line round-trip; a future-series split that partitions `RDATE`/`EXDATE` and copies an edited instance; supported metadata; a timed three-run create/update/sync/cleanup benchmark; and Command Palette Quick Add for both an event and a task. The recurrence checks verify Google's canonical `RRULE`/`EXRULE`/`RDATE`/`EXDATE` set survives an unrelated event rename and that the split returns two independently valid masters. The metadata check verifies Calendar description, location, guest, reminder, timezone, color, visibility, transparency, and a basic recurrence rule, plus HCB task-planning metadata stored in Google Tasks notes. Quick Add intentionally opens the existing full editor, which then saves and syncs the item.
