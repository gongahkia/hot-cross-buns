...
- HCB hoster mode uses encrypted `.hcbvault` snapshot push/pull for a trusted local server/Raspberry Pi/another laptop. Settings and CLI clients can check host status, push the current encrypted vault, or pull a hosted vault after confirmation. The remote host stores encrypted packages, not Google polling state, OAuth credentials, or plaintext planner rows. v1 snapshot pull is destructive and does not merge concurrent writers.
