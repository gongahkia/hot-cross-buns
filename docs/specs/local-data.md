...
- HCB hoster mode uses encrypted `.hcbvault` snapshot push/pull for a trusted local server/Raspberry Pi/another laptop. The remote host stores encrypted packages, not Google polling state, OAuth credentials, or plaintext planner rows. v1 snapshot pull is destructive and does not merge concurrent writers.
