...
enum CacheSchemaMigrator {
    ...
    // v1 share the same field set; v2 adds account catalog fields that are
    // derived from the legacy active account during decode. Future versions
    // add per-step handlers here:
    //
    //     case (0, let tgt):
